...
void validate_detector_map(char *datacfg, char *cfgfile, char *weightfile, float thresh_calc_avg_iou)
{
    ...
    for (i = nthreads; i < m + nthreads; i += nthreads) {
        ...
        for (t = 0; t < nthreads && i + t - nthreads < m; ++t) {
            ...
            char *id = basecfg(path);
			//to draw fp, fn 
			image img_for_fp_fn = copy_image(val_resized[t]);
			//to draw fp, fn 
            float *X = val_resized[t].data;
            network_predict(net, X);
			...
			//nbox = # of detected box
            for (i = 0; i < nboxes; ++i) {

                int class_id;
                for (class_id = 0; class_id < classes; ++class_id) {
                    float prob = dets[i].prob[class_id];
					//3. get detection box after >thresh & nms
                    if (prob > 0) {
                        detections_count++;
                        ...
                        int truth_index = -1;
                        float max_iou = 0;
						//4. compare detection box & ground truth
						//num_labels = # of ground truth box
                        for (j = 0; j < num_labels; ++j)
                        {
                            box t = { truth[j].x, truth[j].y, truth[j].w, truth[j].h };
                        ...
                        if (prob > thresh_calc_avg_iou) {
                            ...
                            if(truth_index > -1 && found == 0) {
                                ...
                                ++tp_for_thresh;
								//add to analize FN
								truth[truth_index - unique_truth_count].found = 1;
								//to draw tp in black
								draw_bbox(img_for_fp_fn, dets[i].bbox, 1, 0, 0, 0);
                            }
							else {
								fp_for_thresh++;
								//to draw fp in red
								draw_bbox(img_for_fp_fn, dets[i].bbox, 1, 1, 1, 1);
							}
                        }
                    }
                }
			...
			for (int aa = 0; aa < num_labels; aa++) {
				box t = { truth[aa].x, truth[aa].y, truth[aa].w, truth[aa].h };
				if (truth[aa].found == 1) {
					//green if found
					draw_bbox(img_for_fp_fn, t, 1, 0, 1, 0);
				}
				else {
					//red if not found
					draw_bbox(img_for_fp_fn, t, 1, 1, 0, 0);
				}
			}
			char map_result_img_path[100];
			sprintf(map_result_img_path, "map_result/%d", image_index);
			save_image(img_for_fp_fn, map_result_img_path);
            ...
            free_image(val[t]);
            free_image(val_resized[t]);
			//delete img_for_fp_fn
			free_image(img_for_fp_fn);
        }
    }
                   ...
                   float hier_thresh, int dont_show, int ext_output, int save_labels)
{
    ...
    while(1){
		...
		//show_image(im, "raw_input");
		//save_image(im, "raw_input");
		//show_image(sized, "resized");
		//save_image(sized, "resized");
